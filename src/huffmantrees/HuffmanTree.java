...
// Insert solution to programming exercise 1, section 6, chapter 6 here
    ...
    public static void main(String[] args) throws FileNotFoundException, UnsupportedEncodingException {
        ...
        String message="tayyip";
        String encodeCode=Htree.encode(message);
        System.out.println("Message to Code: \n"+message+" : "+encodeCode);
    }

    /**
     * Wrapper metot
     * Gelen stringi tek tek harflerine bakarak huffman treede bulunan kodlara gore kodluyorum.
     * @param message gelen string
     * @return kodlanmis mesaj
     */
    public String encode(String message)
    {
        ...
        for (int i=message.length()-1;i>=0; i--)
            encode(message.charAt(i),result,currentTree);
    ...
    /**
     * Wrapper metot icinde cagrilan private encode metodu.
     * @param ch wrapper metotta bulunan stringin tek harfi
     * @param result kodlanmis mesaji yazmak icin parametre olarak gelen StringBuilder.
     * @param currentTree olusturulmus olan agac
     * @return istenen harf bulununca true,bulunmazsa false.
     */
    private boolean encode(char ch,StringBuilder result,BinaryTree<HuffData> currentTree)
    {
        boolean durum=false;
        if(currentTree.isLeaf()){            
            if(currentTree.getData().symbol.equals(ch))
                return true;}
        
        if(currentTree.getData().getSymbol()==null)
        {
            durum=encode(ch,result,currentTree.getLeftSubtree());
                if(durum==true){
                    result.append("0"); return durum;}
            
            durum=encode(ch, result, currentTree.getRightSubtree());
                if(durum==true){
                    result.append("1"); return durum;}
        }
